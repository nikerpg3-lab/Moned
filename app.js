...
// --- Filtrul activ pe istoric (null = toate) ---
...
// --- Setează filtrul activ din click pe card dashboard ---
function setFilter(type) {
  // Click pe cardul activ = dezactivare filtru
  ...
  Object.entries(cardMap).forEach(([key, id]) => {
    ...
    el.classList.remove('card-filter-active');
    if (activeFilter === key) el.classList.add('card-filter-active');
  });

  // Actualizează titlul istoricului
  const labelMap = {
    income:     'Venituri',
    expense:    'Cheltuieli',
    investment: 'Investiții',
  };
  const titleEl = document.getElementById('list-filter-label');
  titleEl.textContent = activeFilter
    ? `Filtrat: ${labelMap[activeFilter]}`
    : 'Toate';

  // Re-randează lista cu filtrul aplicat
...
function addTransaction() {
  ...
  // Actualizează UI (respectă filtrul activ)
